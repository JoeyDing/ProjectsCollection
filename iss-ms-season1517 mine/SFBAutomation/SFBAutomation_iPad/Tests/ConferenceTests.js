...
function VerifyJoinOnlineMeetingFromEvent(){
    ...
    if(TapTableviewCellWithPredicate(MEETINGNAME,GetValueFromKey("LOCID Meetings")) == true){
        ...
        var join_Button = MainWindow.tableViews()[1].buttons()[GetValueFromKey("MEETING_VIEW_JOIN_BUTTON_TEXT")]
        
        if(join_Button.isVisible() == true && join_Button.isValid() == true) {
            
            join_Button.tap();
           
            if(WaitButtonEnabledAndValid(GetValueFromKey("LOCID LIVECALL_VIDEO_BUTTON_VIDEOOFF_ACCESSIBILITY_LABEL")) == true) {
