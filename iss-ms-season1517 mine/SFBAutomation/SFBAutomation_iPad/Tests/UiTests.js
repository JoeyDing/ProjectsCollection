...
UIATarget.onAlert = function onAlert(alert)
{
    ...
    try
    {
        ...
        try
        {
            var window_3 = Target.frontMostApp().windows()[Target.frontMostApp().windows().length - 1] ;
            if(window_3.isValid() == true && window_3.isVisible() == true){
                
                try{
                    window_3.elements()[0].tap();
                }
                catch(error){
                    LogMessage("it is first display");
                }
            }
        }
        catch(error)
